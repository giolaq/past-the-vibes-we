...
cp -R workshop/fixtures/pocket-cinema-inputs \\
  "$WORKSHOP_INPUTS"`,
  memoryShow: `yarn --cwd packages/workshop-harness tsx src/index.ts memory show /tmp/past-the-vibes-pocket-cinema-inputs --json`,
...
const modules = [
  {
    ...
    body: `${concept("Read this first if you have never built an agent harness","You are a React Native developer. You may have never touched an \"agent,\" an \"LLM tool,\" or \"MCP.\" That is fine — nothing here assumes you have. We will port a small RN app to Vega (Amazon's TV OS) without doing it by hand and without just asking an AI to \"please port my app.\" Instead we build a <strong>harness</strong>: a plain TypeScript program that runs a fixed pipeline, lets an AI model <em>propose</em> code inside tight walls, and keeps every dangerous action — writing files, running checks, committing to Git, spending money, talking to the device — for itself.")}
      ...
      ${table(["Term","What it actually is","Your mental model"],[
        ...
        ["<strong>MCP</strong>","<a href=\"https://modelcontextprotocol.io\" target=\"_blank\" rel=\"noopener\">Model Context Protocol</a> — a standard so one program can start another as a \"server\" and call its tools.","Starting a language server (LSP) and querying it, but for arbitrary tools."],
        ["<strong>ADBT</strong>","<a href=\"https://www.npmjs.com/package/@amazon-devices/amazon-devices-buildertools-mcp\" target=\"_blank\" rel=\"noopener\">Amazon Devices Builder Tools</a>, exposed here as an MCP server serving Vega migration docs.","An internal wiki you can query programmatically."],
...
guarded app copy  <--- read-only tools (list/read/search) -----------------+--> Strands Agent --> typed patch {summary, files}
     ...
     HARNESS: reconstruct + hash ADBT reads -> validate paths -> write files -> run checks -> retry -> git commit -> report`,
      ...
      ${note("What is Strands Agents SDK?","<a href=\"https://github.com/strands-agents/harness-sdk\" target=\"_blank\" rel=\"noopener\">Strands</a> is the TypeScript agent runtime used by the live remote path. Both the complete workshop harness and the staged mini-harness pin 1.10.0. It provides the model loop, provider adapters, typed tools, structured output, limits, cancellation, and usage metrics.")}
      ...
      ${steps(["Install Node.js 20 or newer and Git. <a href=\"https://nodejs.org/api/corepack.html\" target=\"_blank\" rel=\"noopener\">Corepack</a> supplies Yarn 4.12.","Clone this repository, run <code>cd past-the-vibes-we</code>, and keep the terminal at that root.","Choose Pocket Cinema unless your own React Native app already runs."])}
      ...
      ${table(["Path","Use it when","Needs"],[["Claude Code","Default. A local live coding-agent run.","<a href=\"https://code.claude.com/docs\" target=\"_blank\" rel=\"noopener\">Claude Code</a> installed and authenticated; ADBT via init-context"],["Strands + Bedrock","Default. The in-process remote executor.","AWS credentials and <a href=\"https://docs.aws.amazon.com/bedrock/\" target=\"_blank\" rel=\"noopener\">Bedrock</a> model access"],["Replay","Fallback when a live model, ADBT, or device is unavailable.","Nothing beyond the installed packages"]])}
      ...
      ${note("Where these come from","Amazon Devices Builder Tools ships the ADBT MCP server, skills, and steering docs as an npm package. <code>init-context</code> writes the MCP config into your agent; <code>check-status</code> confirms it. See the <a href=\"https://developer.amazon.com/docs/vega/0.22/mcp-server.html\" target=\"_blank\" rel=\"noopener\">Vega ADBT setup docs</a>.")}
