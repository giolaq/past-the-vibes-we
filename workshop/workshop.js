...
const modules = [
  {
    ...
    body: `${concept("Read this first if you have never built an agent harness","You are a React Native developer. You may have never touched an \"agent,\" an \"LLM tool,\" or \"MCP.\" That is fine — nothing here assumes you have. We will port a small RN app to Vega (Amazon's TV OS) without doing it by hand and without just asking an AI to \"please port my app.\" Instead we build a <strong>harness</strong>: a plain TypeScript program that runs a fixed pipeline, lets an AI model <em>propose</em> code inside tight walls, and keeps every dangerous action — writing files, running checks, committing to Git, spending money, talking to the device — for itself.")}
      ...
      ${snippet("The boundary, in one diagram",`ADBT (MCP server) <--- adbt_list_documents / adbt_read_document (recorded + hashed) --+
                                                                                     |
guarded app copy  <--- read-only tools (list/read/search) --------------------------+--> Strands Agent --> typed patch {summary, files}
     ...
     HARNESS: validate paths -> write files -> run checks -> retry once -> git commit -> enforce cost -> write report`,
        "The model (via Strands) can <em>list, read, and search</em> the guarded app and <em>call the ADBT read tools itself</em> — but it has no write tool and no shell, and every ADBT read is recorded with a hash. It returns JSON. Everything with consequences stays in <code>packages/workshop-harness/src/port-pipeline.ts</code>.")}
      ...
      <p>The port agent can list, read, and search the guarded app, and during <code>analyze</code> and <code>plan</code> it is also handed the ADBT MCP read tools (<code>adbt_list_documents</code>, <code>adbt_read_document</code>) so it discovers and reads the Vega migration workflows itself — the harness does not pre-pick them. It still gets no shell or write tool, and every ADBT read is recorded with a SHA-256 hash so the run stays auditable. Replay uses recorded model turns and recorded ADBT context, so it needs no live model or MCP server.</p>
...
const result = await agent.invoke(prompt, {
  ...
  },
  {
    ...
    body: `${concept("The production loop","The plan is the human approval boundary. During analyze and plan the model calls ADBT over MCP itself to gather Vega knowledge. Strands proposes a typed patch from read-only project tools. The harness records every ADBT read, applies the patch, checks it, retries once, and commits only verified work.")}${flow([["ADBT MCP","Model reads workflows"],["Record","Hash each read"],["Model","Propose a typed patch"],["Checks","Write, commit, or retry"]])}
      ...
      ${phaseCard({num:"2",name:"plan",tags:[{label:"model",kind:"model"},{label:"live ADBT",kind:"adbt"}],rows:{"Goal":"Plan the Vega TV port: preserved product behavior, Vega replacements, and the exact remote flow.","ADBT step":"The model is handed the ADBT MCP read tools and calls them itself — <code>adbt_list_documents</code> to discover Vega workflows, then <code>adbt_read_document</code> to read the ones it judges relevant. The harness pre-picks nothing; it records each read with a SHA-256 hash into <code>adbt-port-context.json</code>.","Skill injected":"Use the ADBT tools to discover and read the Vega migration workflows you need. Keep facts and assumptions separate, port one vertical slice, record gaps instead of inventing APIs.","Checks":"<code>VEGA_PORT.md</code> contains <code>## TV Flow</code>; <code>NextSteps.md</code> contains <code>ADBT</code> (names its sources).","Inspect":"<code>out/&lt;runId&gt;/app/VEGA_PORT.md</code>, <code>NextSteps.md</code>, <code>adbt-port-context.json</code>, and the commit <code>workshop(plan): ...</code>."}})}
