...
| 9 | `workshop/lessons/09-finish.md` | Run the complete replay command. In a human terminal, add `--tui` and inspect checks/model/tools for different phases; in a non-interactive agent shell, omit `--tui` and inspect `out/final-dashboard/model-logs/` directly. Then answer `workshop/worksheet.md` in the report for a domain of your choice: phases, one mechanical check per phase, approval point, budget, and retained evidence. |

## You are done when

`out/agent-report.md` covers setup plus lessons 1–9, every evidence row above is verified, and every knowledge-check answer is written in your own words.
